...
const LocationPin = (props) => {
    ...
    const subcategory =  markerData.category.children.edges[0].node;
    const mediumImgSize = markerData.featuredImage.mediaDetails.sizes.filter( (element) => element.name == 'thumbnail')
    ...
    const markerContent = (<div className={"labelWrapper"} >
            <div className={"locationPinImageWrapper"}>
                <img className={"locationPinImage"} src={mediumImgSize[0].sourceUrl}/>
            </div>
            <span className={"locationTitle"}>{markerData.title}</span>
            
            <span className={"locationSubtitle"}>{subcategory.name}</span>
        </div>
        );
